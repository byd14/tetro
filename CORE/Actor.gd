...
func collision_move(offset : Vector2, time : float = 1) -> Vector2:
	...
	if v_delta.x != 0:
		if BACKYARD.collision_check(Collision, Collision.Overlaps, Vector2(v_delta.x, 0)):
			while !BACKYARD.collision_check(Collision, Collision.Overlaps, Vector2(sign(v_delta.x), 0)):
				global_position.x += sign(v_delta.x)
			v_delta.x = 0; reminder.x = 0;
			collision_x = true
		global_position.x += v_delta.x
	if v_delta.y != 0:
		if BACKYARD.collision_check(Collision, Collision.Overlaps, Vector2(0, v_delta.y)):
			while !BACKYARD.collision_check(Collision, Collision.Overlaps, Vector2(0, sign(v_delta.y))):
